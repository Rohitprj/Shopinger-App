...
// import axiosInstance from '../utils/AxiosInstance'; // <--- This is your actual axiosInstance import
...
//   // and allowing it to be used in the retry button without dependency issues.
...
//       } else {
//         // Handle API-specific errors (e.g., success: false from backend)
//         setError(response.data.message || 'Failed to fetch addresses: Unknown reason.');
//       }
//     } catch (err) {
//       // Handle network errors (e.g., no internet, server unreachable, 4xx/5xx responses)
...
//   useEffect(() => {
//     fetchAddresses(); // Call the memoized fetch function when the component mounts
//   }, [fetchAddresses]); // Dependency on fetchAddresses (stable due to useCallback)

//   const handleAddAddress = () => {
//     // This is a placeholder. In a real app, you would use React Navigation here.
//     // Alert.alert(
//     //   "Add Address", 
//     //   "You would navigate to a screen to add a new address here.",
//     //   [{ text: "OK" }]
//     // );
//     // Example with React Navigation (if 'navigation' prop is available):
...
//       {addresses.length === 0 ? ( // Only show "No addresses" if not loading and no error, and list is empty
...
//                 {/* Display name, handling potential empty fristname/lastname */}
//                 {address.fristname || address.lastname ? 
//                   `${address.fristname || ''} ${address.lastname || ''}`.trim() 
//                   : 'N/A' // Fallback if both are empty
//                 }
//               </Text>
              
//               <Text>
//                 {/* Display houseNo and street, handling if houseNo is empty */}
//                 {address.houseNo ? `${address.houseNo}, ` : ''}{address.street}
//               </Text>
              
//               <Text>
//                 {/* Display city and district, handling potential empty district */}
...
//               {/* Conditionally render landmark and mobile if they exist and are not empty strings */}
...
//               {/* Display "Default Address" badge if isDefault is true */}
...
//   },
//   retryButton: {
//     backgroundColor: '#ffc107', // Amber color for retry
//     paddingVertical: 10,
//     paddingHorizontal: 20,
//     borderRadius: 5,
//   },
...
import axiosInstance from '../utils/AxiosInstance'; // <--- Your actual axiosInstance import

export default function AllAddresses({ navigation }) { // Ensure navigation prop is received
  const [addresses, setAddresses] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
